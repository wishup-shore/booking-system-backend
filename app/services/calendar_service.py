...
class CalendarService:
    ...
    def get_occupancy_for_date_range(self, start_date: date, end_date: date) -> List[CalendarOccupancy]:
        """Get occupancy data for a date range"""
        # Get all accommodations
        accommodations = self.db.query(Accommodation).options(joinedload(Accommodation.type)).all()
        ...
        while current_date <= end_date:
            ...
            for accommodation in accommodations:
                ...
                for booking in bookings:
                    if (booking.accommodation_id == accommodation.id and
                ...
                accommodation_data = {
                    ...
                    'type_name': accommodation.type.name if accommodation.type else 'Unknown',
            ...
            occupancy_data.append(CalendarOccupancy(
                date=current_date,
                accommodations=accommodations_for_date
            ))
    ...
    def get_calendar_events(self, start_date: date, end_date: date) -> List[CalendarEvent]:
    ...
    def check_accommodation_availability(self, accommodation_id: int, 
    ...
    def get_available_accommodations(self, start_date: date, end_date: date, 
                                   capacity_needed: Optional[int] = None) -> List[Accommodation]:
        """Get all accommodations available for given dates"""
        # Get all accommodations that meet capacity requirements
        accommodations_query = self.db.query(Accommodation).options(joinedload(Accommodation.type))
        
        if capacity_needed:
            accommodations_query = accommodations_query.filter(Accommodation.capacity >= capacity_needed)
        ...
        for accommodation in all_accommodations:
            if self.check_accommodation_availability(accommodation.id, start_date, end_date):
        ...
        for booking in bookings:
            ...
            if booking_start < booking_end:
                ...
                if booking.accommodation and booking.accommodation.price_per_night:
                    total_revenue += float(booking.accommodation.price_per_night) * nights
        
        occupancy_rate = (occupied_nights / total_nights * 100) if total_nights > 0 else 0.0
    ...
    def get_accommodation_schedule(self, accommodation_id: int, 
    ...
    def find_next_available_slot(self, accommodation_id: int, 
                               after_date: date, min_nights: int = 1) -> Optional[Tuple[date, date]]:
